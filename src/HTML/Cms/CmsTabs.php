...
class CmsTabs
{
    ...
    public function  __toString()
    {
        ...
        foreach ($this->getTabs() as $k => $tab) {
            $titles[] = '<li' . ($tab['active'] ? ' class="active"' : '') . '><a href="#' . Converter::nameToHtmlAttribute($tab['title']) . '" data-toggle="tab">' . Converter::charsToNormalTitle($tab['title']) . '</a></li>';
            $contents[] = '<div class="tab-pane fade' . ($tab['active'] ? ' active' : '') . ' in" id="' . Converter::nameToHtmlAttribute($tab['title']) . '">' . $tab['content'] . '</div>';
        }
